        ...
        <div className="space-y-6">
                                    ...
                                    <tbody>
                                        {myAttendanceData.length > 0 ? myAttendanceData.map((record, index) => (
                                            <tr key={record._id} className={`border-b transition-colors ${isDark ? 'border-gray-700 hover:bg-gray-700/50' : `border-blue-100 ${index % 2 === 0 ? 'bg-blue-50/40' : 'bg-indigo-50/30'} hover:bg-blue-100/60`}`}>
                                                ...
                                                <td className={`p-4 text-sm font-medium ${isDark ? 'text-gray-300' : 'text-gray-800'}`}>
                                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                                                        record.attendanceType === 'Office' 
                                                            ? (isDark ? 'bg-blue-900/40 text-blue-300' : 'bg-blue-200 text-blue-900')
                                                            : record.attendanceType === 'WFH'
                                                            ? (isDark ? 'bg-purple-900/40 text-purple-300' : 'bg-purple-200 text-purple-900')
                                                            : (isDark ? 'bg-orange-900/40 text-orange-300' : 'bg-orange-200 text-orange-900')
                                                    }`}>
            ...
            {activeTab === 'users' && (
                <div className={`p-8 rounded-xl transition-all max-w-2xl mx-auto ${
                    isDark ? 'bg-gray-800 border border-gray-700 shadow-xl' : 'bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 shadow-lg'
                }`}>
                    <h3 className={`text-2xl font-bold mb-6 ${isDark ? 'text-white' : 'text-green-900'}`}>
                        Create New Employee
                    </h3>
                    {message && (
            ...
            {activeTab === 'attendance' && (
                <div className={`p-6 rounded-xl ${isDark ? 'bg-gray-800 border border-gray-700 shadow-xl' : 'bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 shadow-lg'}`}>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                        <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-amber-900'}`}>Employee Attendance Management</h3>
                        <div className="flex flex-wrap gap-2">
                            <input type="date" className={`p-2 border rounded-lg text-sm ${isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-amber-300 text-gray-900'}`} value={exportFilters.startDate} onChange={e => setExportFilters({...exportFilters, startDate: e.target.value})} />
                            <input type="date" className={`p-2 border rounded-lg text-sm ${isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-amber-300 text-gray-900'}`} value={exportFilters.endDate} onChange={e => setExportFilters({...exportFilters, endDate: e.target.value})} />
                            <select className={`p-2 border rounded-lg text-sm ${isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-amber-300 text-gray-900'}`} value={exportFilters.attendanceType} onChange={e => setExportFilters({...exportFilters, attendanceType: e.target.value})}>
                                <option value="">All Types</option>
                                <option value="Office">Office</option>
                                <option value="WFH">WFH</option>
                                <option value="Field">Field</option>
                            </select>
                            <button onClick={handleExport} className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-all shadow-lg">Export</button>
                        </div>
                    </div>
